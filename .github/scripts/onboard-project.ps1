...
.DESCRIPTION
    ...
    FULL preset (default, adds):
      - Work Type     (Epic / Story / Task / Bug / Spike)
...
.PARAMETER ProjectTitle
    Display title for the project.  Default: ITL.ControlPlane.Attestation

.PARAMETER RepoName
    Repository to link (format: owner/repo). Also required for milestones + labels.
    Default: ITlusions/ITL.ControlPlane.Attestation

.PARAMETER Preset
    "basic" -- Sprint, Priority, Story Points, Category only.
    "full"  -- basic + Work Type, Effort, Epic, Blocked, Risk.  (Default)

.PARAMETER CategoryOptions
    Comma-separated Category options.
    Default: attestation categories
...
.PARAMETER CreateViews
    Output a views setup guide with field IDs and filter URLs.

.PARAMETER SkipIfExists
    Reuse an existing project instead of aborting when it already exists.

.EXAMPLE
    # Full Agile onboard (idempotent re-run on existing project #35)
    .\onboard-project.ps1 -SkipIfExists -CreateLabels -CreateViews

.EXAMPLE
    # Full onboard with 6 sprint milestones
    .\onboard-project.ps1 -SprintCount 6 -CreateLabels -CreateViews -SkipIfExists
#>

[CmdletBinding()]
param(
    [string] $Org              = "ITlusions",
    [string] $ProjectTitle     = "ITL.ControlPlane.Attestation",
    [string] $RepoName         = "ITlusions/ITL.ControlPlane.Attestation",
...
# ---------------------------------------------------------------------------
# 8. Create sprint milestones (optional)
# ---------------------------------------------------------------------------
if ($SprintCount -gt 0 -and $RepoName -eq "") {
    Write-Warning "Skipping milestones: -RepoName is required."
    $SprintCount = 0
}
...
Write-Host "  Sprint (Iteration), Priority, Story Points, Category"
if ($Preset -eq "full") {
    Write-Host "  Work Type, Effort, Epic, Blocked, Risk" -ForegroundColor Yellow
}
Write-Host ""
Write-Host "Next steps:" -ForegroundColor Yellow
Write-Host "  1. Sprint field -> project Settings -> configure duration + start date"
if ($CreateViews) {
    Write-Host "  2. Create the views listed above in the GitHub UI"
} else {
    ...
    Write-Host "     Backlog (table, group Priority), Epics (table, group Epic)"
}
Write-Host ""
